...
CKEDITOR.plugins.add('ezer', {
  ...
  init: function (editor) {
    ...
    editor.on('paste', function (evt) {
      ...
      var EditHtml= editor.element.$.parentNode.retrieve('Ezer'), LabelDrop= 0, ok= 1;
...
function admin(a) {
  var logo=  $('logo'),
      work=  $('work'),
      dolni= $('dolni');
  if ( !a ) work.setStyles({height:'inherit'});
  if ( logo ) logo.setStyles({zIndex:a?99999:0});
  if ( dolni ) dolni.setStyles({display:a?'block':'none'});
  return 1;
}
// ------------------------------------------------------------------------------------- cms_cid_pid
...
function vyber() {
  ...
  checks= $('vyber');
  if ( checks ) {
    checks= checks.getElements('input[type=checkbox]');
    checks.each(function(el) {
      if ( el.checked )
        list+= (list?',':'')+el.getAttribute('data-value');
    });
  }
  return list;
}
